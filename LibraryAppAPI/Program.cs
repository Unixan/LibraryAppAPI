    ...
    public class Program
    {
        public static void Main(string[] args)
        {
           ...
           app.MapGet("/libraryUser/{id}", async (Guid id) =>
            ...
            app.MapPut("/libraryUser/{user.UserID}", async (User user) =>
            {
                var conn = new SqlConnection(connStr);
                const string sql = "UPDATE [User] SET LoanCard = @LoanCard WHERE UserID = @UserID";
                var changeUserLoanCard = await conn.ExecuteAsync(sql, user);
                return changeUserLoanCard;
            ...
            app.MapPut("/libraryBook/{book.BookId}", async (Book book) =>
            {
                var conn = new SqlConnection(connStr);
                const string sql = "UPDATE [Book] SET LoanedToID = @LoanedToId, LoanedOnDate = @LoanedOnDate WHERE BookId = @BookId";
                var bookLoanedToStatus = await conn.ExecuteAsync(sql, book);
                return bookLoanedToStatus;
            });
